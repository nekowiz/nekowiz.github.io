	...
	$.ajax({
		...
		for (var question_index in data_arr) {
			...
			if (question_id != q_id) {
				...
				// 產生編輯表單
				var result_html = "題目類型：<select id='gen'><option>生活常識</option><option>動漫與遊戲</option><option>文科</option><option>理科</option><option>演藝</option><option>體育</option><option>&lt;待填&gt;</option></select><hr />題目難度：<select id='color'><option>1色</option><option>2色</option><option>3色</option><option>&lt;待填&gt;</option></select><hr /><input type='text' id='problem' placeholder='題目內容' size='100' /><hr /><input type='text' id='answer' placeholder='答案' size='30' /><hr /><button id='cancel_edit'>取消修改</button>&nbsp;<button id='confirm_edit'>確認修改</button>";
				$("#edit").html(result_html).show(1000);
				$("#gen").val(question_gen);
				$("#color").val(question_color);
				...
				$("#confirm_edit").on('click', function() {
					...
					// 產生新題目的該行
					var new_question_line = q_id.toString() + "|" + $("#gen").val() + "|" + $("#problem").val() + "|" + $("#answer").val() + "|" + $("#color").val() + "\n";
	...
	$.ajax({
		...
		for (var question_index in data_arr) {
			...
			if (question_id != q_id) {
				...
				// 產生編輯表單
				var result_html = "<input type='text' id='problem' placeholder='題目內容' size='100' /><hr /><input type='text' id='answer' placeholder='答案' size='30' /><hr /><button id='cancel_edit'>取消修改</button>&nbsp;<button id='confirm_edit'>確認修改</button>";
				$("#edit").html(result_html).show(1000);
				$("#problem").val(question_text);
				$("#answer").val(question_answer);
				// 取得圖片 img
				var img_elem = $("<img height='398px' width='239px' title='" + question_filename + "' />");
				var img_url = "http://zh.nekowiz.wikia.com/api.php?action=query&prop=imageinfo&titles=File:" + question_filename + "&iiprop=url&format=json";
				...
				$("#confirm_edit").on('click', function() {
					...
					// 產生新題目的該行
					var new_question_line = q_id.toString() + "|" + $("#problem").val() + "|" + $("#answer").val() + "|" + question_filename + "\n";
...
function search_problem() {
	...
	var search_min_word = parseInt($("#search_min_word").val());
	...
	if (type == "select") {
		...
		if (question == "") {
			...
			var db_result = taffy_dbs["normal"]( function () {
				for (var question_index in question_split) {
					...
					if (this.question.toLowerCase().indexOf(question_key) == -1) return false;
				}
				return true;
			});
			var result_count = db_result.count();
			db_result = db_result.limit(20);
			var search_result_html = "<table class='article-table'><tr><td colspan='6'>搜尋出 " + result_count.toString() + " 個結果，超過 20 筆將會只顯示前 20 筆搜尋結果。</td></tr><tr><th>題號</th><th>類型</th><th>難度</th><th>題目</th><th>答案</th><th></th></tr>";
		...
		if (question == "") {
			...
			db_result.each(function (record,recordnumber) {
				...
				var img_url = "http://zh.nekowiz.wikia.com/api.php?action=query&prop=imageinfo&titles=File:" + record["image_filename"] + "&iiprop=url&format=json";
				$.getJSON(img_url, function(data) {
					for (var first in data.query.pages) {break;}
					if (data.query.pages[first].imageinfo == undefined) {
						return;
