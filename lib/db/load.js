...
function search_problem() {
	...
	if (type == "select") {
		...
		if (question == "") {
			...
			var question_split = question.split(" ");
			var db_result = taffy_dbs["dairy"]( function () {
				for (var question_index in question_split) {
					var question_key = question_split[question_index];
					if (this.question.search(question_key) == -1) return false;
				}
			...
			var search_result_html = "<table class='article-table'><tr><th>題號</th><th>圖片</th><th>題目</th><th>答案</th><th></th></tr>";
			db_result.each(function (record,recordnumber) {
				search_result_html += "<tr><td>" + record["id"] + "</td><td>" + record["image_filename"] + "</td><td>" + record["question"] + "</td><td>" + record["answer"] + "</td><td><button onclick='editQuestionDaily(" + record["id"] + ")'>修改</button></td></tr>";
			});
			search_result_html += "</table>";
			$("#result").html(search_result_html);
		}
