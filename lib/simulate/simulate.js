var Card, Enemy, attack_penal, cards, check_all_cards_loaded, check_stage_clear, check_stage_fail, combo, current_enemies, current_stage, disable_attacks, enable_attacks, enable_log, enemies_one_turn_pass, inital_enemy_data, load_card_info_to_input, load_cards, load_enemy_info_to_input, monsters, msg_log, panel_color, play_stage, player_action, player_attack, props, quests, randomNum, randomTurn, stage_all_clear, stages, start;
    ...
    if (this.current_hp !== 0) {
      ...
      for (i = _i = 1, _ref = this.attack_info.atk_times; 1 <= _ref ? _i <= _ref : _i >= _ref; i = 1 <= _ref ? ++_i : --_i) {
        if (this.attack_info.target_num === 1) {
          ...
          msg_log(this.name + " ratio: " + attack_ratio + ", combo: " + combo + ", atk: " + atk + ", damage: " + atk_value + ", target: " + enemy.name);
