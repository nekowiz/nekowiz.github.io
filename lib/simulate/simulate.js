var Card, Enemy, cards, check_all_cards_loaded, check_stage_clear, combo, current_enemies, current_stage, inital_enemy_data, load_card_info_to_input, load_cards, load_enemy_info_to_input, monsters, panel_color, play_stage, player_action, player_attack, props, randomNum, randomTurn, stages, start;
...
load_card_info_to_input = function() {
  var atk_index, hp_index, i, _i, _results;
  _results = [];
  for (i = _i = 1; _i <= 6; i = ++_i) {
    hp_index = "#card" + i + "_hp";
    atk_index = "#card" + i + "_atk";
    $(hp_index).val(cards[i - 1].current_hp);
    _results.push($(atk_index).val(cards[i - 1].max_atk));
  }
  return _results;
};
...
load_enemy_info_to_input = function() {
  ...
  for (_i = 0, _len = current_enemies.length; _i < _len; _i++) {
    ...
    _results.push($(hp_index).val(enemy.current_hp));
  }
  return _results;
};
