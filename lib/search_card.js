function input_card_name_keyup_fun() {
	...
	db_result.each(function (record,recordnumber) {
		search_result_html += "<span class='search_result_small_img' data-card_id='" + record["id"] + "' data-small_filename='" + record["small_filename"] + "'></span>" + record["name"] + "<br />";
	});
	...
	$(".search_result_small_img").each(function() {
		...
		$.getJSON(small_url, function(data) {
			...
			this_element.append("<a href='add_card(" + this_element.data("card_id") + ");'><img height='60px' width='60px' src='" + url + "' /></a>");
		});
	});
}
