...
function findLeader() {
	...
	var cardId = parseInt($("div.LeaderFinder input[name='cardId']").val());
	if(isNaN(cardId)){
		cardId = "";
	}
	$("div.LeaderFinder input[name='cardId']").val(cardId);
	if ($("div.LeaderFinder input[name='cardId']").val() === "") {
...
function randomLeader() {
	...
	getLeaderData(function (leaderTextData) {
		...
		var randomindexs = [];
		var randomResults = [];
		var maxRandomCount = (Math.floor(Math.random() * 10) + 1) * 3;
...
function showRandomResults(results) {
	$(results).each(function (index, result) {
		...
		getLeaderImage(result.cardId, function (data) {
